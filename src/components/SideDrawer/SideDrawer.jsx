...
function TemporaryDrawer({ cart, removeFromCart }) {
    ...
    const list = (anchor) => (
        ...
        >
            <Button variant='contained' sx={{ width: '100%', borderRadius: 0 }} size='large' color={'secondary'} disableRipple>Items List</Button>
            <List>

                {cart.cart.map((text, index) => (
                    <ListItem key={text._id} disablePadding>

                        <Button sx={{ marginLeft: '5px' }} variant='text' size='small' color={'secondary'} disableRipple>
